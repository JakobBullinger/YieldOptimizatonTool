...
def main_app():
    ...
    if st.button("Auswerten & Aggregieren"):
        ...
        rename_map = {
            ...
            "teile": "Teile",
            # "vol_ausgang" wird nicht mehr benötigt – wir ersetzen sie durch "Durchmesser"
        ...
        # Final: Entferne "Volumen_Ausgang" und berechne "Durchmesser" pro Auftrag
        if "Volumen_Ausgang" in grouped.columns:
            grouped.drop("Volumen_Ausgang", axis=1, inplace=True)

        # berechnet wird und in den Folgezellen 0 enthält.
        grouped["Durchmesser"] = 0.0
        for auftrag, grp in grouped.groupby("Auftrag"):
            idx = grp.index[0]  # Erste Zeile des Auftrags
        ...
        # Finales Spalten-Layout
        final_output_cols = [
            "Auftrag", "Dimension", "Stämme", "Volumen_Eingang", "Durchschn_Stämme", "Teile",
            "Durchmesser", "Brutto_Volumen", "Brutto_Ausschuss", "Netto_Volumen",
            "Brutto_Ausbeute", "Netto_Ausbeute", "CE", "SF", "SI", "IND", "NSI", "Q_V", "Ausschuss"
        ]
