...
def main_app():
    ...
    # --- Zeitfenster für MicroTec-Abfrage definieren ---
    default_date = df_microtec["Datetime"].iloc[0].date()
    st.markdown("### 3) Zeitfenster definieren (MicroTec)")
    orders_final = {}
    order_instances = {}
    ...
    # --- Laufzeit definieren (zusätzlich) ---
    st.markdown("### 4) Laufzeit definieren")
    orders_runtime = {}
    for order_number, ukey_list in order_instances.items():
        for idx, ukey in enumerate(ukey_list):
            st.markdown(f"#### Laufzeit für Auftrag {order_number} – Instanz {idx+1} von {len(ukey_list)}")
            c1, c2 = st.columns(2)
            ls_h = c1.number_input(f"Laufzeit Start-Stunde {ukey}", 0, 23, 6, 1)
            ls_m = c1.number_input(f"Laufzeit Start-Minute {ukey}", 0, 59, 0, 1)
            le_h = c2.number_input(f"Laufzeit End-Stunde {ukey}", 0, 23, 12, 1)
            le_m = c2.number_input(f"Laufzeit End-Minute {ukey}", 0, 59, 0, 1)
            runtime_start = datetime.combine(default_date, time(ls_h, ls_m))
            runtime_end = datetime.combine(default_date, time(le_h, le_m))
            orders_runtime[ukey] = (runtime_start, runtime_end)

    def compute_yield(volume, vol_in_liters):
        # Berechnung wie bisher – Achtung: Einheit klären!
        if vol_in_liters == 0:
            return 0
    ...
    if st.button("Auswerten & Aggregieren"):
        ...
        # Aggregationsschritt
        # Gruppiere jetzt nach unique_key, Auftrag und Dimension, um die zusätzlichen Laufzeit-Daten zu erhalten
        ...
        numeric_cols = [
            ...
            "si_0_iv_cbm", "si_i_ii_cbm", "ind_ii_iii_cbm", "nsi_i_iii_cbm",
            "ass_iv_cbm", "waste_cbm"
        ]
        for c in numeric_cols:
            if c not in df_agg.columns:
                df_agg[c] = 0
            df_agg[c] = pd.to_numeric(df_agg[c], errors="coerce")
        # Gruppierung inklusive unique_key
        agg_dict = {
            "stämme": "sum",
            "vol_eingang": "sum",
        ...
        # --- Neue Spalte "Durchmesser" berechnen ---
        # Diese Spalte wird in der ersten Zeile jedes Auftrags berechnet.
        grouped["Durchmesser"] = 0.0
        # --- Neue Spalte "Laufzeit_Minuten" berechnen ---
        grouped["Laufzeit_Minuten"] = 0.0

        # Wir gehen jetzt pro unique_key vor (jeder unique_key entspricht einer Auftrag-Instanz)
        for ukey, grp in grouped.groupby("unique_key"):
            ...
            # Berechnung Laufzeit in Minuten aus orders_runtime
            if ukey in orders_runtime:
                rt_start, rt_end = orders_runtime[ukey]
                runtime_minutes = (rt_end - rt_start).total_seconds() / 60
            else:
                runtime_minutes = 0
            grouped.loc[idx, "Laufzeit_Minuten"] = round(runtime_minutes, 2)

        # Entferne die Spalte "unique_key" aus dem finalen Output
