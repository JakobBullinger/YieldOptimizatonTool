...
synonyms = {
    ...
    "17x95": "17x100",
    "23x100": "23x103",
    "47x220": "47x221",  # Beispiel für 47x220
}
...
def normalize_dimension(dim_str):
    ...
    Entfernt Leerzeichen, 'mm', Kommas etc.
    """
    if pd.isna(dim_str):
        return "Unknown"
    dim_str = (
...
############################
# 2) Funktion zum Extrahieren der ersten 5 Ziffern des Auftrags
############################
def extract_order_number(auftrag_str):
    """
    Sucht am Anfang des Strings nach 5 Ziffern (Regex: ^(\d{5})),
    z.B. aus "10432 - 2x47x135" wird "10432".
    Falls nicht gefunden, wird der Originalstring zurückgegeben.
    """
    if not isinstance(auftrag_str, str):
        return auftrag_str
    auftrag_str = auftrag_str.strip()
    m = re.match(r'^(\d{5})', auftrag_str)
    if m:
        return m.group(1)
    return auftrag_str
...
def load_and_prepare_data(filepath):
    ...
    # Dimension normalisieren + vereinheitlichen
...
def to_excel_bytes(df):
    """
    Hilfsfunktion, um ein DataFrame in einen Excel-Byte-Stream zu konvertieren.
    """
    output = io.BytesIO()
...
def main_app():
    ...
    if pdf_file is not None:
        ...
        if df_prod is not None:
            ...
            # Konvertiere numeric_cols zu float
            ...
            # Erstes Excel: Nur das PDF-Parsing
            ...
            # -------------------------------------------------------------
            # Hauptzeilen (unterkategorie == "") sammeln
            # -------------------------------------------------------------
            ...
            for i, row_ in df_main.iterrows():
                ...
                # WICHTIG: Schon hier normalisieren und vereinheitlichen,
                # damit wir später beim CSV-Mergen dieselben Werte haben
                ...
                # Zero-Width-Key, falls Auftragsnummer doppelt
    ...
    if st.button("Auswerten"):
        ...
        for order, params in orders_final.items():
            ...
            dims = params["dimensions"]  # bereits vereinheitlicht
            ...
            for idx, row in result.iterrows():
                ...
                final_rows.append({
                    # Wichtig: Wir nennen das hier NICHT 'auftrag', 
                    # damit wir später kein Konflikt haben
                    "auftrag_orig": order,
                    "Dimension": dimension,
                    "Warentyp": warentyp,
        ...
        # Download der reinen MicroTec-Auswertung
        ...
        # -------------------------------------------------------------
        # 5. Zusammenführen mit dem ursprünglichen PDF-Parsing
        # -------------------------------------------------------------
        st.markdown("### 5. Zusammenführen mit dem ursprünglichen PDF-Parsing + MicroTec Statistik")

        # a) Auch df_prod['unterkategorie'] normalisieren + vereinheitlichen
        ...
        # b) Extrahiere in beiden DF die ersten 5 Ziffern in eine neue Spalte 'auftrag_num'
        df_prod["auftrag_num"] = df_prod["auftrag"].apply(extract_order_number)

        # final_df_for_merge: Umbenennen, damit wir KEIN 'auftrag' hier haben
        final_df_for_merge = final_df.rename(columns={
            "auftrag_orig": "auftrag_orig",
            "Dimension": "unterkategorie",
            "Brutto_Ausbeute (%)": "Brutto_Ausbeute",
            "Netto_Ausbeute (%)": "Netto_Ausbeute",
            "Vol_Eingang (in m³)": "Vol_Eingang_m3"
        })

        # c) Erzeuge 'auftrag_num' in final_df_for_merge
        final_df_for_merge["auftrag_num"] = final_df_for_merge["auftrag_orig"].apply(extract_order_number)

        # d) Merge auf ["auftrag_num", "unterkategorie"]
        merged_df = pd.merge(
            df_prod,
            final_df_for_merge,
            on=["auftrag_num", "unterkategorie"],
            how="left",
            suffixes=("_left", "_right")
        )

        # e) Spaltenreihenfolge festlegen
        final_columns = [
            "auftrag",          # Original aus df_prod
            "unterkategorie",
            "stämme",
            "vol_eingang",
            "durchschn_stammlänge",
        ...
        # Falls einzelne Spalten fehlen, legen wir sie leer an
        for col in final_columns:
            if col not in merged_df.columns:
                merged_df[col] = ""
        merged_df = merged_df[final_columns]

        # f) NaNs als leere Strings
        merged_df.fillna("", inplace=True)

        st.dataframe(merged_df)

        # g) Download der zusammengeführten Tabelle
        excel_merged = to_excel_bytes(merged_df)
        st.download_button(
            label="Download Zusammengeführte Excel",
            data=excel_merged,
            file_name=f"Produktivitaet_MicroTec_merged_{default_date}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
