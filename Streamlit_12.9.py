...
def main_app():
    ...
    for order_number, ukey_list in order_instances.items():
        for idx, ukey in enumerate(ukey_list):
            ...
            # Eingabefeld für Laufzeit in Minuten (direkt als Zahl)
            runtime_minutes = st.number_input(f"Laufzeit (in Minuten) für Auftrag {order_number} - Instanz {idx+1}", min_value=0.0, value=0.0, step=1.0)
            st.markdown("##### MicroTec Zeit")
            c1, c2 = st.columns(2)
            mt_sh = c1.number_input(f"Start-Stunde (MicroTec) {ukey}", 0, 23, 6, 1)
            mt_sm = c1.number_input(f"Start-Minute (MicroTec) {ukey}", 0, 59, 0, 1)
            mt_eh = c2.number_input(f"End-Stunde (MicroTec) {ukey}", 0, 23, 12, 1)
            mt_em = c2.number_input(f"End-Minute (MicroTec) {ukey}", 0, 59, 0, 1)
            microtec_start = datetime.combine(default_date, time(mt_sh, mt_sm))
            microtec_end = datetime.combine(default_date, time(mt_eh, mt_em))
            orders_time[ukey] = {"runtime": runtime_minutes, "microtec": (microtec_start, microtec_end)}
    ...
    if st.button("Auswerten & Aggregieren"):
        ...
        # Entferne "Volumen_Ausgang" falls vorhanden und füge neue Spalten hinzu:
        # "Durchmesser" und "Laufzeit_Minuten" werden nur in der ersten Zeile eines jeden Auftrags berechnet.
        ...
        for ukey, grp in grouped.groupby("unique_key"):
            ...
            # Verwende den direkt eingegebenen Laufzeitwert aus orders_time
            if ukey in orders_time:
                runtime_minutes = orders_time[ukey]["runtime"]
            else:
                runtime_minutes = 0
            grouped.loc[idx, "Laufzeit_Minuten"] = round(runtime_minutes, 2)
        # Entferne "unique_key" aus dem finalen Output
