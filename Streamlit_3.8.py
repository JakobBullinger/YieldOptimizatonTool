...
############################
# HIER: Hilfsfunktion, die an den Auftragsnamen
# unsichtbare Zero-Width-Spaces hängt,
# damit ein Auftrag, der doppelt vorkommt, NICHT
# überschrieben wird (Dictionary-Schlüssel bleibt eindeutig).
############################
def make_invisible_key(order_name: str, index: int) -> str:
    return order_name + ("\u200B" * index)
    # \u200B ist ein Zero-Width Space und wird in der UI nicht sichtbar.
    # So kann "10421 - 3x47x135" mehrfach als "Key" existieren,
    # bleibt aber im Streamlit-Output optisch unverändert.
...
def main_app():
    ...
    if pdf_file is not None:
        ...
        if df_prod is not None:
            ...
            # ----------------------------------------------------------------
            # Anstatt .unique() => wir iterieren nur über die "Hauptauftrags-Zeilen"
            # (unterkategorie == ""), in der Reihenfolge des PDF. Das ermöglicht
            # doppelte Aufträge an verschiedenen Stellen.
            # ----------------------------------------------------------------
            df_main = df_prod[df_prod["unterkategorie"] == ""].reset_index(drop=True)
            for i, row in df_main.iterrows():
                order_name = row["auftrag"]
                
                # Alle Unterkategorien einsammeln, die zu diesem Auftrag gehören:
                # => Aus dem Original-Code: "group = df_prod[df_prod['auftrag'] == order]"
                group = df_prod[df_prod["auftrag"] == order_name]
                ...
                # Key mit Zero-Width-Spaces, damit doppelte order_name nicht überschrieben werden
                unique_key = make_invisible_key(order_name, i)
                orders_from_pdf[unique_key] = list(dict.fromkeys(dims))
            
            st.markdown("**Automatisch ermittelte Dimensionen pro Auftrag (in Reihenfolge):**")
            for key, dims in orders_from_pdf.items():
                # key enthält evtl. Zero-Width-Spaces, die man nicht sieht
                st.write(f"{key}: {dims}")
